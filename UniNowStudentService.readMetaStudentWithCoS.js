...
const PERIOD_SZ_HISKEY_ID = 7; // SZ/ Gesamter Semesterzeitraum
...
var personMainData = findMainDataByPersonId(personId);
...
if(studentAuditor) {
    ...
    var studentMainData = findStudentDataByPersonId(personId, lang);
...
/* function to determine if a person is a student or auditor (=GasthÃƒÆ’Ã‚Â¶rer)*************************/
...
function findMainDataByPersonId(personId) {
        ...
        if (!firstname && !surname && !birthcity && !birthdate) {
                return null;
        }

        sharedLogger.debug("findMainDataByPersonId found by personId=" + personId + ":" + " firstname= " + firstname + " surname= " + surname + " birthcity= " + birthcity + " birthdate= " + birthdate);
...
function findStudentDataByPersonId(personId, lang) {
        ...
        for (var i=0; i<degreeProgramProgresses.size(); i++) {
               ...
               var degree = degrees.byId[courseOfStudyResponse.longValue("courseOfStudy/degreeId")];

               var studentstatus = getAllStudentstatus(lang);
               var studentstatusByUniquename = studentstatus.byUniquename[degreeProgramProgress.stringValue("studentstatus")];

               var coS = new CourseOfStudy(
               idAsStringOrNull(courseOfStudyResponse.longValue("courseOfStudy/id")),
               (courseOfStudyResponse.stringValue("courseOfStudy/shorttext") + "").toString(),
               (courseOfStudyResponse.stringValue("courseOfStudy/defaulttext") + "").toString(),
               (courseOfStudyResponse.stringValue("courseOfStudy/longtext") + "").toString(),
        ...
        sharedLogger.debug("Number of DPPÂ´s collected: " + dPPs.length);
